...
export default function Chat({ petType, userName, onAppointment, chatState, onChatStateChange }) {
  const { haptic, hapticNotification } = useMax();
  ...
  // Set pet type for API calls
  useEffect(() => {
    if (petType?.name) setCurrentPetType(petType.name);
  }, [petType]);
  ...
  return (
    <div className="tg-chat">
      ...
      {/* Appointment button — in-screen (Max has no MainButton) */}
      {hasEnoughDialog && (
        <div className="tg-appoint-bar">
          <button className="tg-appoint-btn" onClick={() => { haptic('medium'); onAppointment(); }}>
            🏥 Записаться на приём
          </button>
        </div>
      )}
