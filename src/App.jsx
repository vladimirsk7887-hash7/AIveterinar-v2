...
import { callAI, parseMeta, mergeCard, checkServerKey } from "./lib/ai";
...
export default function App() {
  ...
  // Close sidebar on mobile when switching pets
  const selectPet = (id) => {
    setActivePetId(id);
  ...
  const createPet = async (petType) => {
    ...
    setActivePetId(id);
    setLoading(true);
    const reply = await callAI([{ role: "user", content: initMsg }]);
  ...
  const sendMessage = async (text) => {
    ...
    setLoading(true);
    const reply = await callAI(updatedMessages.map((m) => ({ role: m.role, content: m.content })));
    const { meta, visibleText } = parseMeta(reply);
      ...
      <div className="home-container" style={{
        ...
        {!serverReady && (
          <div style={{
            ...
            Сервер не настроен. Убедитесь, что OPENAI_API_KEY задан в переменных окружения.
          </div>
        )}
  ...
  return (
    <div style={{
      ...
      {showAppointment && <AppointmentModal pet={activePet} messages={activePet?.messages || []} onClose={() => setShowAppointment(false)} />}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        {/* Mobile: appointment button above input */}
