    ...
    <div>
      ...
      {conversations.length === 0 ? (
        <div className="empty-state">
          ...
          <div style={{ fontSize: 12, marginTop: 8, color: '#546E7A' }}>Диалоги появятся, когда пользователи начнут общаться через виджет</div>
        </div>
      ) : (
      ...
      {(page > 1 || hasMore) && (
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 20 }}>
          ...
          <button className="btn btn-outline" disabled={!hasMore} onClick={() => load(page + 1)}>
            Далее
          </button>
        </div>
      )}
